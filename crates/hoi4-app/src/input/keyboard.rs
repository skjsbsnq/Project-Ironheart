...
enum PlayingEscapeAction {
    CancelFrontlinePainter,
    ExitConstructionMode,
    CloseCounterMenu,
    CollapseExpandedStacks,
    ClosePopup,
    RequestLawDetailClose,
    CloseDetailImmediately,
    RequestPoliticsClose,
    ClosePrimaryImmediately,
    ExitGame,
}

#[derive(Debug, Clone, Copy)]
struct PlayingEscapeState {
    frontline_active: bool,
    construction_mode: bool,
    counter_menu_open: bool,
    expanded_stacks_open: bool,
    popup_open: bool,
    law_detail_open: bool,
    detail_open: bool,
    politics_panel_open: bool,
    primary_panel_open: bool,
}

fn resolve_playing_escape_action(state: PlayingEscapeState) -> PlayingEscapeAction {
    if state.frontline_active {
        PlayingEscapeAction::CancelFrontlinePainter
    } else if state.construction_mode {
        PlayingEscapeAction::ExitConstructionMode
    } else if state.counter_menu_open {
        PlayingEscapeAction::CloseCounterMenu
    } else if state.expanded_stacks_open {
        PlayingEscapeAction::CollapseExpandedStacks
    } else if state.popup_open {
        PlayingEscapeAction::ClosePopup
    } else if state.law_detail_open {
        PlayingEscapeAction::RequestLawDetailClose
    } else if state.detail_open {
        PlayingEscapeAction::CloseDetailImmediately
    } else if state.politics_panel_open {
        PlayingEscapeAction::RequestPoliticsClose
    } else if state.primary_panel_open {
        PlayingEscapeAction::ClosePrimaryImmediately
    } else {
        PlayingEscapeAction::ExitGame
    }
}
        ...
        if btn_state == ElementState::Pressed {
            ...
            match code {
                KeyCode::Escape => match self.view.game_phase {
                    ...
                    GamePhase::Playing => {
                        let action = resolve_playing_escape_action(PlayingEscapeState {
                            frontline_active: self.interaction.frontline_painter.mode
                                != PainterMode::Idle,
                            construction_mode: self.ui_state.construction_mode.is_some(),
                            counter_menu_open: self
                                .interaction
                                .counter_right_click_province
                                .is_some(),
                            expanded_stacks_open: !self.interaction.expanded_stacks.is_empty(),
                            popup_open: self.ui_state.active_popup.is_some(),
                            law_detail_open: matches!(
                                self.ui_state.active_detail_panel,
                                Some(hoi4_ui::ActiveDetailPanel::Law { .. })
                            ),
                            detail_open: self.ui_state.active_detail_panel.is_some(),
                            politics_panel_open: matches!(
                                self.ui_state.open_panel,
                                Some(InGamePanel::Politics | InGamePanel::Laws)
                            ),
                            primary_panel_open: self.ui_state.open_panel.is_some(),
                        });
                        match action {
                            PlayingEscapeAction::CancelFrontlinePainter => {
                            ...
                            PlayingEscapeAction::RequestLawDetailClose => {
                                if let Some(s) = &self.state {
                                    hoi4_ui::detail_panel::request_law_political_ideas_close(
                                        &s.ui.ctx,
                                    );
                            ...
                            PlayingEscapeAction::ExitGame => event_loop.exit(),
                        }
                    }
                },
                ...
                KeyCode::F1 => {
                    ...
                    debug_commands::log_bool_toggle(
                        "gui overlay",
                        self.render_toggles.debug_overlay,
                    );
                    changed = true;
                }
                KeyCode::F2 => {
                ...
                KeyCode::F5 => {
                    if let Some(s) = &mut self.state {
                        ...
                        if shift_held {
                            self.render_toggles.postprocess_debug_view =
                                s.post_process.cycle_debug_view();
                ...
                KeyCode::F6 => {
                    self.render_toggles.terrain_debug_view =
                        self.render_toggles.terrain_debug_view.next();
                    debug_commands::log_value(
                        "terrain debug view",
                ...
                KeyCode::F8 => {
                    ...
                    if shift_held {
                        self.render_toggles.map_quality_preset =
                            self.render_toggles.map_quality_preset.next();
                        if let Some(s) = &mut self.state {
                ...
                KeyCode::F10 => {
                    self.render_toggles.water_debug_view =
                        self.render_toggles.water_debug_view.next();
                    debug_commands::log_value(
                        "water debug view",
                ...
                KeyCode::KeyV => {
                    self.render_toggles.border_debug_view =
                        self.render_toggles.border_debug_view.next();
                    debug_commands::log_value(
                        "border debug view",
...
mod tests {
    ...
    fn escape_state() -> PlayingEscapeState {
        PlayingEscapeState {
            frontline_active: false,
            construction_mode: false,
            counter_menu_open: false,
            expanded_stacks_open: false,
            popup_open: false,
            law_detail_open: false,
            detail_open: false,
            politics_panel_open: false,
            primary_panel_open: false,
        }
    }
    ...
    fn gate11_escape_requests_law_detail_close_before_primary_panel() {
        ...
        assert_eq!(
            resolve_playing_escape_action(state),
            PlayingEscapeAction::RequestLawDetailClose
        );
    }
    ...
    fn gate11_escape_keeps_legacy_non_politics_close_immediate() {
        ...
        assert_eq!(
            resolve_playing_escape_action(detail_state),
            PlayingEscapeAction::CloseDetailImmediately
        );

        let mut primary_state = escape_state();
        primary_state.primary_panel_open = true;
        assert_eq!(
            resolve_playing_escape_action(primary_state),
            PlayingEscapeAction::ClosePrimaryImmediately
        );
    }
