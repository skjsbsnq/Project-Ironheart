...
fn probe_action_row_and_bg() {
    ...
    let ctx = match VanillaGuiRuntimeContext::load_result(
        COUNTRY_DIPLOMACY_DESCRIPTOR.required_gui_files,
    ) {
        Ok(c) => c,
        Err(_) => {
            eprintln!("no vanilla ctx");
            return;
        }
    };
    let root = ctx
